...
class Modem(object):
    def __init__(self, serialPort, mode='ymodem', program="rzsz"):
        self.logger = logging.getLogger('Modem')
        self.serialPort = serialPort
        self.reader = self.sender_read
        self.writer = self.sender_write
        self.mode   = mode
    ...
    def sender_read(self, size, timeout=3):
        self.serialPort.serialport.timeout = timeout
        return self.serialPort.serialport.read(size) or None

    def sender_write(self, data, timeout=3):
        self.serialPort.serialport.writeTimeout = timeout
        return self.serialPort.serialport.write(data)

    def abort(self, count=2, timeout=60):
        for _ in range(count):
            self.sender_write(CAN, timeout)

    def send(self, stream, retry=100, timeout=10, callback=None, info=None):
        try:
            packet_size = dict(
                xmodem    = 128,
                xmodem1k  = 1024,
        ...
        if self.mode.startswith("ymodem"):
            ...
            while True:
                ...
                self.sender_write(header + data + checksum)
                self.logger.debug("[Sender]: Info block sent")

                # Blocking may occur here, the reader needs to have a timeout mechanism
                char = self.sender_read(1, timeout)
                if char == ACK:
                    error_count = 0
                    break
                else:
        ...
        if self.mode.startswith("ymodem"):
            ...
            while True:
                if error_count >= retry:
                    ...
                    if not self.sender_write(CRC):
                        self.logger.debug("[Receiver]: Error, write failed, sleeping for {}".format(delay))
                        time.sleep(delay)
                        error_count += 1
                else:
                    crc_mode = 0
                    if not self.sender_write(NAK):
                        self.logger.debug("[Receiver]: Error, write failed, sleeping for {}".format(delay))
                        time.sleep(delay)
                        error_count += 1

                char = self.sender_read(1, timeout=3)
                if char is None:
            ...
            while True:
                ...
                if not (seq1 == seq2 == 0):
                    ...
                    # skip this packet
                    self.sender_read(packet_size + 1 + crc_mode)
                    self.logger.warning("[Receiver]: a wrong packet dropped")
                else:
                    self.logger.debug("[Receiver]: Read a packet")
                    data = self.sender_read(packet_size + 1 + crc_mode, timeout)
                ...
                while True:
                    data = self.sender_read(1, timeout=1)
                    if data is None:
                        break
                self.sender_write(NAK)
                char = self.sender_read(1, timeout)
                continue
